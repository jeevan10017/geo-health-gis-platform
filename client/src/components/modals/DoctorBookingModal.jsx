...
import { X, Calendar, Hospital, CheckCircle, Info } from 'lucide-react';
...
const DoctorBookingModal = ({ doctorId, hospitalId: initialHospitalId, userLocation, onClose }) => {
    ...
    useEffect(() => {
        const fetchDoctor = async () => {
            if (!doctorId || !userLocation) {
                setError("Missing doctor ID or user location.");
                setIsLoading(false);
                return;
            }
            setIsLoading(true);
    ...
    const selectedHospital = doctor?.hospitals.find(h => h.hospital_id === parseInt(selectedHospitalId));
    ...
    const getTravelSuggestion = () => {
        ...
        const travelTimeMinutes = Math.round(((selectedHospital.distance_meters / 1000) / 40) * 60) + 5;
            ...
            <div className="bg-white rounded-lg shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
                ...
                {doctor && !bookingConfirmed && (
                    <div className="p-4 space-y-4">
                            ...
                            <>
                                ...
                                <div>
                                    ...
                                    <DatePicker
                                        ...
                                        minDate={new Date()}
                                        className="w-full p-2 border rounded-md"
                                        calendarClassName="rounded-md border"
                                    ...
                                    {!availableDays.includes(getDayOfWeek(selectedDate)) && (
                                        <p className="text-sm text-amber-600 mt-1">
                                            Doctor is not available on this day. Available on: {availableDays.map(d => dayMap[d]).join(', ')}
                                        </p>
                                    )}
                                </div>
                        ...
                        <button 
                            ...
                            className="w-full bg-indigo-600 text-white font-bold py-3 rounded-md disabled:bg-slate-300 hover:bg-indigo-700"
                        >
                            Confirm Booking
                        </button>
                    </div>
                ...
                {bookingConfirmed && selectedHospital && (
                    <div className="p-6 text-center space-y-4">
                        ...
                        <button onClick={onClose} className="w-full bg-slate-600 text-white font-bold py-3 rounded-md hover:bg-slate-700">
                            Close
                        </button>
                    </div>
                )}
